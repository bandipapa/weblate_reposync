...
class Main:
    ...
    def _wl_request(self, url, method = "GET", payload = None, need_result = False):
        ...
        try:
            with urllib.request.urlopen(req) as resp:
                ...
                if status < 200 or status > 299:
                    raise ValueError("HTTP status {} {}".format(status, resp.reason))
        ...
        if exc:
            raise FatalError("weblate: {} {}: {}".format(method, url, exc))
...
class Addon:
    name = Field(ty = Field.TY.Header) # TODO: Check for uniqueness or can we create the same addon multiple times?
    settings_json = Field(ty = Field.TY.String)
...
class Repository:
    name = Field(ty = Field.TY.Header)
    url = Field(ty = Field.TY.String)
...
class Config:
    repo_root = Field(ident = "repository_root", ty = Field.TY.String)
    wl_url = Field(ident = "weblate_url", ty = Field.TY.String)
    wl_token = Field(ident = "weblate_token", ty = Field.TY.String)
    component_slug_prefix = Field(ty = Field.TY.String, default = "auto-") # TODO: Add support for suffix.
    import_path = Field(ty = Field.TY.String, default = None)
